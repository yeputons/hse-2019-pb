...
// ...
pthread_mutex_lock(&m);
// Следующие две строки в любом порядке, но лучше так.
some_condition = true;
...
\begin{frame}{Демо}
	См. \t{03-prod-cons.cpp}
	\begin{enumerate}
		\item На каждую введённую строчу отзывается второй поток: сначала сразу, а потом через две секунды.
		\item Если во время ожидания второго потока ввести новую строчку, то на неё второй поток тоже среагирует.
		\item Если во время ожидания ввести две новых строчки, то будет обработана только последняя.
...
\begin{frame}[t,fragile]{Проблема-2}
	Поменяем \t{fgets} и \t{pthread\_mutex\_lock} местами.
\begin{minted}{cpp}
while (true) {  // Под Linux видно проблему
    pthread_mutex_lock(&m);
    fgets(str, sizeof str, stdin);
    str_available = true;
    pthread_cond_signal(&cond);
